...
- `grep -c "items-center text-center" src/components/sections/Contacto.astro` returns 0 — confirms bare (non-responsive) centering classes were removed, not duplicated

## Tasks

- [x] **T01: Apply responsive Tailwind classes for mobile Contacto layout** `est:15m`
...
This slice is a CSS-only layout fix with no runtime behavior, JS logic, or server-side code. Observability is limited to build-time and visual inspection:

- **Build-time signal:** `npm run build` exit code — a non-zero exit indicates a template syntax error in the Astro component.
- **Visual inspection surface:** Browser DevTools responsive mode at 390×844 viewport — inspect the `#contacto` section to verify `text-align`, `align-items`, and child widths.
- **Failure visibility:** If responsive classes are misapplied, the layout will visibly break at either mobile or desktop breakpoints. No silent failures are possible since Tailwind purges unused classes at build time — a typo in a class name results in the style simply not applying, which is visually obvious.
- **No redaction constraints:** This slice contains no secrets, tokens, or PII.
